    ...
    public void actionPerformed(ActionEvent e)
    {
        if (e.getSource() == okButton)
        {
            ...
            case Constants.OPTION_ADD_STOCK:
                ...
                {
                    // Update product stock and persist via Shop
                    int add = Integer.parseInt(textFieldStock.getText());
            ...
            case Constants.OPTION_REMOVE_PRODUCT:
                ...
                {
                    // call shop delete (persist and remove locally)
                    if (product.getId() > 0) {
                        shop.deleteProduct(product.getId());
                    } else {
                        // fallback: remove from local inventory if no ID
